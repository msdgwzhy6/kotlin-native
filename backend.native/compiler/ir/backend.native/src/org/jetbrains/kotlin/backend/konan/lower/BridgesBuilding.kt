        ...
        irBuilder.run {
            ...
            if (typeSafeBarrierDescription != null) {
                ...
                for (i in valueParameters.indices) {
                    if (!typeSafeBarrierDescription.checkParameter(i))
                        continue
                    var type = target.valueParameters[i].type
                    if (type.isValueType() && !descriptor.overriddenDescriptor.valueParameters[i].type.isValueType())
                        type = getBoxType(type.correspondingValueType!!)
                    if (type != context.builtIns.nullableAnyType) {
